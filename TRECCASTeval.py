...
class ConvSearchEvaluation:
    ...
    def eval(self, result, topic_turn_id):
        ...
        #Tries to get the relevance judgments        
        ...
        true_pos = np.intersect1d(result['_id'], query_rel_docs, return_indices=True)
        true_pos = np.sort(true_pos[1])
        n_docs_found = 0
        for doc_ind in true_pos:
            n_docs_found += 1
            recall_var.append(n_docs_found/total_relevant)
            precision_var.append(n_docs_found/(doc_ind +1))
        ...
        return [p10, recall, ap, ndcg5], precision_var, recall_var
